...
DBoxConnect.prototype.getDirectLink = function(path, fn) {
  ...
  this.dbox_client.media(path, function(status, reply){
    if (status != 200) return fn(reply.error);
    cache[path] = { url: reply.url, expires: new Date(reply.expires) };
    fn(null, reply.url);
  });
}

DBoxConnect.prototype.put = function(source, target, fn) {
  var self = this;
...
DBoxConnect.prototype.get = function(path, fn) {
  this.dbox_client.get(path, function(status, reply, metadata){
    if (status == 200) fn(null, reply, metadata)
    else  fn(JSON.parse(reply.toString()).error);
  });
}
/*
 * Middlewares
...
DBoxConnect.prototype.upload = function(opt) {
  opt = opt || {};
  ...
  return function(req, res, next) {
    ...
    self.put(file.path, target, function(err, reply) {
